...
import { inv, multiply, matrix, ones, subset, index } from 'mathjs';
...
export function calcYMatrix (admitancesRe, admitancesIm) {
  ...
  invA_PQUnknown = inv(A_PQUnknown);
}
 ...
 * @return {tf.Scalar[]} `[I0, V1, I2]` `I0`:発電所出力電流[pu], `V1`:変電所2次側電圧[pu], `I2`:需要家消費電流[pu]
 */
export function flowDemandUnknown (V0, I1, V2) {
  for (let i=0; i<3; i++) {
  ...
  const x = multiply(invA_PQUnknown, b);
  ...
  // const V1 = tf.div(tf.sub(I1, tf.add(tf.mul(Y[1][0], V0), tf.mul(Y[1][2], V2))), Y[1][1]);
  // const I0 = tf.add(tf.mul(Y[0][0], V0), tf.add(tf.mul(Y[0][1], V1), tf.mul(Y[0][2], V2)));
  // const I2 = tf.add(tf.mul(Y[2][0], V0), tf.add(tf.mul(Y[2][1], V1), tf.mul(Y[2][2], V2)));
  // return [I0, V1, I2];
}
